// WikiLeirão - Estádios Page JavaScript
class EstadiosPage {
    constructor() {
        this.estadios = [];
    ...
    setupFilters() {
        ...
        filtersContainer.className = 'estadios-filters';
        filtersContainer.innerHTML = `
            <div class="filters-header">
                <h3>Explorar Estádios</h3>
                <div class="view-toggle">
                    <button class="view-btn active" data-view="cards">🏟️ Cards</button>
                    <button class="view-btn" data-view="map">🗺️ Mapa</button>
            ...
            <div class="filter-controls">
                <div class="filter-group">
                    ...
                    <select id="category-filter">
                        ...
                        <option value="grandes">Grandes Estádios (50.000+)</option>
                        <option value="modernos">Estádios Modernos</option>
                        <option value="historicos">Estádios Históricos</option>
                    </select>
                </div>
                <div class="filter-group">
                    <label>Região:</label>
                ...
                <div class="filter-group">
                    ...
                    <select id="capacity-filter">
                        ...
                        <option value="small">Até 25.000</option>
                        <option value="medium">25.000 - 50.000</option>
                        <option value="large">50.000 - 75.000</option>
                        <option value="huge">Mais de 75.000</option>
                    </select>
                </div>
                <div class="filter-group">
                    ...
                    <select id="sort-filter">
                        ...
                        <option value="inaugurated">Ano de Inauguração</option>
                        <option value="city">Cidade</option>
                    </select>
                </div>
                <button id="clear-filters-estadios" class="clear-btn">Limpar Filtros</button>
            </div>
            <div class="quick-stats">
                <div class="stat-item">
                    <span class="stat-number" id="total-estadios">0</span>
                    <span class="stat-label">Estádios</span>
                </div>
                <div class="stat-item">
                    <span class="stat-number" id="total-capacity">0</span>
                    <span class="stat-label">Capacidade Total</span>
                </div>
                <div class="stat-item">
                    <span class="stat-number" id="avg-capacity">0</span>
                    <span class="stat-label">Capacidade Média</span>
                </div>
            </div>
        `;
    ...
    setupFilterEventListeners() {
        ...
        const clearButton = document.getElementById('clear-filters-estadios');
        const viewButtons = document.querySelectorAll('.view-btn');

        categoryFilter.addEventListener('change', () => this.applyFilters());
        regionFilter.addEventListener('change', () => this.applyFilters());
        capacityFilter.addEventListener('change', () => this.applyFilters());
        ...
        clearButton.addEventListener('click', () => {
            ...
            document.getElementById('search-estadios').value = '';
            this.applyFilters();
        });
    ...
    setupSearch() {
        ...
        searchContainer.className = 'search-estadios-container';
        searchContainer.innerHTML = `
            <div class="search-input-wrapper">
                <input type="text" 
                       id="search-estadios" 
                       placeholder="Buscar estádio por nome, cidade, time..." 
                       autocomplete="off" />
                <button class="search-btn">🔍</button>
            </div>
            <div class="search-filters-quick">
                <span class="filter-label">Busca rápida:</span>
                <button class="quick-search" data-search="copa do mundo">Copa do Mundo</button>
                <button class="quick-search" data-search="libertadores">Libertadores</button>
                <button class="quick-search" data-search="arena">Arenas Modernas</button>
                <button class="quick-search" data-search="histórico">Históricos</button>
            </div>
        `;
        ...
        const searchInput = document.getElementById('search-estadios');
    ...
    // Aplicar filtros
    applyFilters() {
        const searchTerm = document.getElementById('search-estadios').value.toLowerCase();
        const categoryFilter = document.getElementById('category-filter').value;
        const regionFilter = document.getElementById('region-filter').value;
        const capacityFilter = document.getElementById('capacity-filter').value;
        const sortFilter = document.getElementById('sort-filter').value;

        this.filteredEstadios = this.estadios.filter(estadio => {
            // Busca por texto
            const matchesSearch = !searchTerm || 
                estadio.name.toLowerCase().includes(searchTerm) ||
                estadio.city.toLowerCase().includes(searchTerm) ||
                estadio.teams.some(team => team.toLowerCase().includes(searchTerm)) ||
                estadio.description.toLowerCase().includes(searchTerm) ||
                estadio.events.some(event => event.toLowerCase().includes(searchTerm));

            // Filtro por categoria
            const matchesCategory = categoryFilter === 'all' || estadio.category === categoryFilter;

            // Filtro por região
            const matchesRegion = regionFilter === 'all' || this.getRegion(estadio.state) === regionFilter;

            // Filtro por capacidade
            let matchesCapacity = true;
            if (capacityFilter !== 'all') {
                const capacity = estadio.capacity;
                switch (capacityFilter) {
                    case 'small':
                        matchesCapacity = capacity <= 25000;
                        break;
                    case 'medium':
                        matchesCapacity = capacity > 25000 && capacity <= 50000;
                        break;
                    case 'large':
                        matchesCapacity = capacity > 50000 && capacity <= 75000;
                        break;
                    case 'huge':
                        matchesCapacity = capacity > 75000;
                        break;
                }
            }

            return matchesSearch && matchesCategory && matchesRegion && matchesCapacity;
        });

        this.sortEstadios(sortFilter);
        this.renderEstadios();
        this.updateStats();
        this.animateResults();
    }
    ...
    getRegion(state) {
        ...
        return regions[state] || 'Outros';
    }

    sortEstadios(criteria) {
        this.filteredEstadios.sort((a, b) => {
            switch (criteria) {
                case 'name':
                ...
                case 'inaugurated':
                    return a.inaugurated - b.inaugurated;
                case 'city':
