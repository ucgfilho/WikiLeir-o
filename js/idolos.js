// WikiLeirão - Ídolos Page JavaScript
class IdolosPage {
    constructor() {
        this.idolos = [];
        this.filteredIdolos = [];
    ...
    setupFilters() {
        ...
        filtersContainer.className = 'idolos-filters';
        filtersContainer.innerHTML = `
            <div class="filters-header">
                <h3>Filtrar Ídolos</h3>
            ...
            <div class="filter-controls">
                ...
                <button id="clear-filters-idolos" class="clear-btn">Limpar Filtros</button>
            </div>
            <div class="quick-stats">
                <div class="stat-item">
                    <span class="stat-number" id="total-idolos">0</span>
                    <span class="stat-label">Ídolos</span>
                </div>
                <div class="stat-item">
    ...
    setupFilterEventListeners() {
        ...
        const clearButton = document.getElementById('clear-filters-idolos');
        const viewButtons = document.querySelectorAll('.view-btn');

        serieFilter.addEventListener('change', () => this.applyFilters());
        positionFilter.addEventListener('change', () => this.applyFilters());
        sortFilter.addEventListener('change', () => this.applyFilters());

        clearButton.addEventListener('click', () => {
            serieFilter.value = 'all';
            positionFilter.value = 'all';
            sortFilter.value = 'name';
            document.getElementById('search-idolos').value = '';
            this.applyFilters();
        });
    ...
    setupSearch() {
        ...
        searchContainer.className = 'search-idolos-container';
        searchContainer.innerHTML = `
            <div class="search-input-wrapper">
                <input type="text" 
                       id="search-idolos" 
                       placeholder="Buscar ídolo por nome, clube, posição..." 
                       autocomplete="off" />
        ...
        const filtersContainer = document.querySelector('.idolos-filters');
        if (filtersContainer) {
            filtersContainer.appendChild(searchContainer);
        }

        const searchInput = document.getElementById('search-idolos');
        ...
        document.addEventListener('click', (e) => {
            if (!e.target.closest('.search-idolos-container')) {
    ...
    showSearchSuggestions(query) {
        ...
        const suggestions = this.idolos.filter(idolo => 
            idolo.name.toLowerCase().includes(query.toLowerCase()) ||
            idolo.club.toLowerCase().includes(query.toLowerCase()) ||
            idolo.position.toLowerCase().includes(query.toLowerCase())
        ).slice(0, 5);

        const suggestionsContainer = document.getElementById('search-suggestions');
        
        if (suggestions.length > 0) {
            suggestionsContainer.innerHTML = suggestions.map(idolo => `
                <div class="suggestion-item" data-idolo="${idolo.id}">
                    <img src="${idolo.image}" alt="${idolo.name}" loading="lazy" />
                    <div class="suggestion-info">
                        <strong>${this.highlightText(idolo.name, query)}</strong>
                        <span>${idolo.club} - ${idolo.position}</span>
                    </div>
                </div>
            `).join('');
            ...
            suggestionsContainer.querySelectorAll('.suggestion-item').forEach(item => {
                item.addEventListener('click', () => {
                    const idoloId = item.dataset.idolo;
                    this.goToIdolo(idoloId);
                });
            });
        } else {
            this.hideSearchSuggestions();
    ...
    // Aplicar filtros
    applyFilters() {
        const searchTerm = document.getElementById('search-idolos').value.toLowerCase();
        const serieFilter = document.getElementById('serie-filter').value;
        const positionFilter = document.getElementById('position-filter').value;
        const sortFilter = document.getElementById('sort-filter').value;

        this.filteredIdolos = this.idolos.filter(idolo => {
            const matchesSearch = !searchTerm || 
                idolo.name.toLowerCase().includes(searchTerm) ||
                idolo.club.toLowerCase().includes(searchTerm) ||
                idolo.position.toLowerCase().includes(searchTerm);

            const matchesSerie = serieFilter === 'all' || idolo.serie === serieFilter;
            const matchesPosition = positionFilter === 'all' || idolo.position === positionFilter;

            return matchesSearch && matchesSerie && matchesPosition;
        });

        this.sortIdolos(sortFilter);
        this.renderIdolos();
        this.updateStats();
        this.animateResults();
    }
